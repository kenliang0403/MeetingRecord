...
| cmd | 调用方 | 用途 |
|---|---|---|
| `status` | recorder-web（dashboard 状态卡 / `/api/status`）<br>recorder-asr-bridge（查 `meeting_id` 写 per-meeting jsonl，10s 缓存）<br>ctrl_query.py CLI | 返回完整运行状态（`in_call`/`main_sending`/`meeting_id`/`gk_registered`/...） |
| `dial` | recorder-web（管理员主动呼出） | 主动呼 MCU 或某终端 |
| `hangup` / `clear_call` | recorder-web | 挂断当前通话 |
| `start_video` / `stop_video` | recorder-web `/api/control/start_video` | 控制本端主流发送 |
| `start_presentation` / `stop_presentation` | recorder-web `/api/control/start_presentation` | H.239 主动演示 |
| `audio_levels` | recorder-web `/api/levels`（VU 表） | 实时 RMS/Peak dBFS |
| `config` | 运维诊断 | 返回当前运行配置 |
| `faststart_one` / `faststart_all` | 运维 | mp4 faststart 重写 |

**协议详细字节格式 + 完整字段** → [api.md §2](api.md#2-recorder-core-controlserver-tcp-9001)
