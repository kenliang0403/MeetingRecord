...
| **POST** | `/recordings/<m>/delete` | rmtree 会议目录 | 永久删除（mp4+字幕+纪要）；正在录制的会议返回 409；写审计日志 |
...
## 4.5 recorder-cleanup.timer（录像保留策略）

定时删除超期录像，防磁盘满。**不监听端口**，是 systemd timer + oneshot service。

| 项 | 值 |
|---|---|
| systemd units | `recorder-cleanup.timer`（OnCalendar 每天 03:00）+ `recorder-cleanup.service`（oneshot） |
| 脚本 | `/opt/recorder/recorder-core/scripts/cleanup_recordings.py`（纯标准库，`/usr/bin/python3`） |
| 配置 | `config.json` 的 `cleanup` 节：`{enabled:false, retention_days:180}`，Web `/config` 页可改 |
| 依赖 | TCP `127.0.0.1:9001` `status`（查正在录制的 meeting_id 以跳过） |
| 删除规则 | 只删 `output_dir` 下目录名以 YYYYMMDD 开头的、超 `retention_days` 的会议；按目录名日期判断（非 mtime）；跳过在录会议 |

详见 [operations.md §5.0](operations.md#50-自动清理推荐--recorder-cleanuptimer)。
